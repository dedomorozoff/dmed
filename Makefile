...
rpm: build-linux-amd64 ## Build .rpm package (x86_64)
	...
	echo 'mkdir -p %{buildroot}%{_bindir} %{buildroot}%{_mandir}/man1' >> dist/rpm/SPECS/dmed.spec
	echo 'install -D -m 755 %{_topdir}/SOURCES/dmed %{buildroot}%{_bindir}/dmed' >> dist/rpm/SPECS/dmed.spec
	echo 'install -D -m 644 %{_topdir}/SOURCES/dmed.1 %{buildroot}%{_mandir}/man1/dmed.1' >> dist/rpm/SPECS/dmed.spec
	echo '' >> dist/rpm/SPECS/dmed.spec
	echo '%files' >> dist/rpm/SPECS/dmed.spec
	echo '%{_bindir}/dmed' >> dist/rpm/SPECS/dmed.spec
	echo '%{_mandir}/man1/dmed.1*' >> dist/rpm/SPECS/dmed.spec
