...
- [ ] Rope-структура вместо `[][]rune` (если упрёмся в производительность)
      — фундамент готов: `internal/rope` (сбалансированное дерево фрагментов,
      O(log n) insert/delete/access, рандомная сверка со срезом). Осталось
      интегрировать в `Buffer` (замена `lines [][]rune` на rope).
